...
def test_flat_parameters_are_perturbed_in_distro():
    ...
    assert (
        rb_coef0_std > 0
    ), f"rb_coef0 should vary across samples, got std={rb_coef0_std}"
    assert (
        rs_coef0_std > 0
    ), f"rs_coef0 should vary across samples, got std={rs_coef0_std}"
    ...
    assert all(
        rb_coef0_min <= val <= rb_coef0_max for val in all_rb_coef0
    ), f"rb_coef0 values should be within [{rb_coef0_min}, {rb_coef0_max}]"
    assert all(
        rs_coef0_min <= val <= rs_coef0_max for val in all_rs_coef0
    ), f"rs_coef0 values should be within [{rs_coef0_min}, {rs_coef0_max}]"

    print(
        f"✅ rb_coef0 varies: {min(all_rb_coef0):.3f} to {max(all_rb_coef0):.3f} (std={rb_coef0_std:.3f})"
    )
    print(
        f"✅ rs_coef0 varies: {min(all_rs_coef0):.3f} to {max(all_rs_coef0):.3f} (std={rs_coef0_std:.3f})"
    )
...
def test_flat_parameters_affect_carbon_functions():
    ...
    assert (
        rb_coeffs_different or rb_times_different
...
def test_flat_parameters_affect_model_output():
    ...
    # Create two models with manually set contrasting flat parameter values
    ...
    # Create models with the contrasting parameter sets
    model1 = CarbonCycleModel(pamset_emiconc_base, pamset_carbon1)
    model2 = CarbonCycleModel(pamset_emiconc_base, pamset_carbon2)
    ...
    assert (
        co2_atm1 > PREINDUSTRIAL_CO2_CONC
    ), "Model 1 should show CO2 increase from emission pulse"
    assert (
